...
export const Viewmeetfrompres = () => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await axios.get('https://project-backend-2i9b.onrender.com/President/presmeet');
  ...
  return (
    ...
    <div className='text-center font-serif text-[20px] font-bold '><h2>MEETINGS FROM PRESIDENT</h2></div>
        ...
        <tbody>
          {currentItems.map((meeting, index) => (
            <tr key={index} className={`${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} text-center ${index % 2 === 0 ? 'dark:bg-gray-900' : 'dark:bg-gray-800'} border-b dark:border-gray-700`}>
              ...
              <td className="">{convertToAMPMFormat(meeting.time)}</td>
